...
export const filteredAndSortedResponses = (responses: CannedResponse[], filter: CannedResponseFilterType): CannedResponse[] =>
...
export const searchInItem = (item: CannedResponse, search: string): boolean => {
  ...
  return (
    searchInString(text, search) || searchInString(tags.join(' '), search) || searchInString(createdBy || '', search)
  );
};